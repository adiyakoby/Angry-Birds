...
{
public:
    using FuncType = std::unique_ptr<OBJTYPE>(*)(std::shared_ptr<World>, const sf::Vector2f& , const sf::Vector2f&);

    static ObjectFactory<OBJTYPE>& instance();
    bool registerType(std::string type, FuncType func);

    std::unique_ptr<OBJTYPE> create(std::string type, std::shared_ptr<World> world,  sf::Vector2f position, const sf::Vector2f& size);
...
template <typename OBJTYPE>
std::unique_ptr<OBJTYPE> ObjectFactory<OBJTYPE>::create(std::string type, std::shared_ptr<World> world, sf::Vector2f position, const sf::Vector2f& size) 
{
