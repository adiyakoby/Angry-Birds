...
BlueBird::BlueBird(std::shared_ptr<World> world, const sf::Vector2f& position, const sf::Vector2f& size, const int& index)
    : Bird(world, position, size, index), m_world{ world}, isSplit{false}, m_state{normal} {
...
static auto registerItYBlueBird = ObjectFactory<Bird>::instance().registerType(
    ...
    {
        return std::make_unique<BlueBird>(world, position, size, index);
    }
);
